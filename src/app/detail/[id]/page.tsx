        ...
        <div className="w-full h-full ">
            ...
            <div className="w-full h-max md:flex">
                <div className="w-full md:w-1/4 p-4 h-max">
                    ...
                    <div className="h-[300px] md:h-[600px] overflow-scroll shadow-md">
                ...
                <div id="detail" className="w-full md:w-3/4 p-10">
                    ...
                    <div className={styles.mappingItem}>
                        {
                            data.genres.map((item, i) => {
                                return <Chip key={i} text={item.name} />
                            })
                        }
                    </div>
                    ...
                    <div className={styles.mappingItem}>
                        {
                            data.production_companies.map((item, i) => {
                                return <Chip key={i} text={`${item.name} (${item.origin_country})`} />
                            })
                        }
                    </div>
                    <h1><b>Countries</b></h1>
                    <div className={styles.mappingItem}>
                        {
                            data.production_countries.map((item, i) => {
                                return <Chip key={i} text={item.iso_3166_1} />
                            })
                        }
                    </div>
                    ...
                    <div className={styles.mappingItem}>
                        <Link className={styles.linkItem} href={data.homepage}><Chip text="Official" /></Link>
                        <Link className={styles.linkItem} href={`https://www.imdb.com/title/${data.imdb_id}`}><Chip text="IMDB" /></Link>
                    </div>
